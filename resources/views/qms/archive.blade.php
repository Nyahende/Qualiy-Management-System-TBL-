...
<body>
   ...
   @if(Session::get('addarchive'))
     {{Session::get('addarchive')}}
   @endif
   @if(Session::get('deletearchive'))
   ...
   <div id="soptabbedpanel">
      ...
      <div id="qcsops" >
          <h1 class="sopdepheader">ARCHIVES</h1>
          <div class="adsopformdiv">
              <form action="{{route('archive.upload')}}" method="POST" class="adsopform" enctype="multipart/form-data">
                  @csrf
             ...
             <div class="actionstable">
                 <table class="archive-table">
                     ...
                     <tr>
                         <td>{{$item->id}}</td>
                         <td>{{$item->Name}}</td>
                         <td><a href="viewarchive/{{$item->File}}">View</a></td>
                         <td><a href='downloadarchive/{{$item->File}}'> Download</a></td>
                         <td>{{$item->Date}}</td>
                         <td><a href="{{'deletearchive/'.$item->id}}">Delete</a></td>
                     </tr>
                     @endforeach
                 </table>
             </div>
