...
<head>
    ...
    <title>QMS | Planned Actions</title>
</head>
...
<body>
   ...
   @if(Session::get('addaction'))
     {{Session::get('addaction')}}
   @endif
   @if(Session::get('deleteaction'))
   ...
   <div id="soptabbedpanel">
      ...
      <div id="qcsops" >
          <h1 class="sopdepheader">PLANNED ACTIONS</h1>
          <div class="adsopformdiv">
              <form action="{{route('uploadfile')}}" method="POST" class="adsopform" enctype="multipart/form-data">
                  @csrf
             ...
             <div class="actionstable">
                 <table>
                     ...
                     <tr>
                         <td>{{$item->id}}</td>
                         <td>{{$item->Name}}</td>
                         <td><a href="viewaction/{{$item->File}}">View</a></td>
                         <td><a href='downloadaction/{{$item->File}}'> Download</a></td>
                         <td>{{$item->Date}}</td>
                         <td><a href="{{'deleteaction/'.$item->id}}">Delete</a></td>
                     </tr>
                     @endforeach
                 </table>
             </div>
