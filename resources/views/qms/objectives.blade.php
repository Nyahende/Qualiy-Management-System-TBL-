...
<body>
   ...
   @if(Session::get('addobjective'))
     {{Session::get('addobjective')}}
   @endif
   @if(Session::get('deleteobjective'))
   ...
   <div id="soptabbedpanel">
      ...
      <div id="qcsops" >
          <h1 class="sopdepheader">OBJECTIVES</h1>
          <div class="adsopformdiv">
              <form action="{{route('objective.upload')}}" method="POST" class="adsopform" enctype="multipart/form-data">
                  @csrf
             ...
             <div class="actionstable">
                 <table>
                     ...
                     <tr>
                         <td>{{$item->id}}</td>
                         <td>{{$item->Name}}</td>
                         <td><a href="viewobjective/{{$item->File}}">View</a></td>
                         <td><a href='downloadobjective/{{$item->File}}'> Download</a></td>
                         <td>{{$item->Date}}</td>
                         <td><a href="{{'deleteobjective/'.$item->id}}">Delete</a></td>
                     </tr>
                     @endforeach
                 </table>
             </div>
